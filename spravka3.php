...
<?php
 ...
 if (!empty($client_id))
 {
 $sql="SELECT s.sale_id, m.merch_id, m.merch_number, e.title, e.year, a.artist_name, e.price, c.client_name, t.merchtype_name, s.sale_date
       FROM Sales s
	   JOIN Client c
 ...
 echo "<tr><th>Продажба</th><th>Стока</th><th>Бройка</th><th>Заглавие</th><th>Година</th><th>Изпълнител</th><th>Цена</th><th>Клиент</th><th>Вид стока</th><th>Дата на продажба</th></tr>";
 while($row = mysqli_fetch_array($result)){
 echo "<tr><td>".$row['sale_id']."</td><td>".$row['merch_id']."</td><td>".$row['merch_number']."</td><td>".$row['title']."</td><td>".$row['year']."</td><td>".$row['artist_name']."</td><td>".$row['price']."</td><td>".$row['client_name']."</td><td>".$row['merchtype_name']."</td><td>".$row['sale_date']."</td></tr>"; }
